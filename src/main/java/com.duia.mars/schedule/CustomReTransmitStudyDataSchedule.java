...
@Component
@Configurable
public class CustomReTransmitStudyDataSchedule implements ISchedule {

    private static final Logger logger = LoggerFactory.getLogger(CustomReTransmitStudyDataSchedule.class);
    ...
    @Scheduled(cron = "0 39 16 4 4 ?")    // 自定义时间，必须精确到[年-月-日 时:分]
    public void schedule() {
        ...
        // todo for test
        beforeDay = "2018-04-03";

        String beginTime = beforeDay + " 00:00:00";
        String endTime = beforeDay + " 23:59:59";

        ScheduleRecord record = new ScheduleRecord(Common.ScheduleType.TYPE_RE_TRANSMIT_STUDY_DATA_INDEX, now, CustomReTransmitStudyDataSchedule.class.getName());
        ...
        try {
            ...
            if (null != sendResultStatus && TransmitStudyDataHistory.SEND_RESULT_STATUS_SUCCESS == sendResultStatus) {
                ...
                updateScheduleRecordAsSuccess(record, beginMil);
            ...
            // 新建发送历史，第二次 todo not save
            TransmitStudyDataHistory history = null;//saveNewHistory(transmitStudyDataList);
     ...
     * 更新调度状态为成功
     * @param record 调度记录
     * @param beginMil 开始计时时间戳，毫秒
     */
    private void updateScheduleRecordAsSuccess(ScheduleRecord record, long beginMil) {

        record.setStatus(ScheduleRecord.STATUS_SUCCESS);

        record.setExecuteTime(String.valueOf(System.currentTimeMillis() - beginMil));
